...
/*
함수 : 전달된 컬럼값을 읽어들여서 함술흘 실행한 결과를 반환

-단일행 함수: N개의 값을 읽어서 N개의 결과괎 리턴 (매 행마다 함수 실행 결과 반환)
...
SELECT lower('Welocome To MySQL'), upper('Welcome To MySQL');
...
SELECT emp_name, email, "변경전" ,replace(email,'kh.or.kr', 'gmail.com') 변경후
FROM employee;
...
* 날짜단위 : YEAR(연), QUARTER(분기), MONTH(월), 
           WEEK(주), DAY(일), HOUR(시간), MINUTE(분), SECOND(초)
*/
...
if(dept_code is null, '부서업음', '부서있음') 
FROM employee;
...
-- 사원명, 직급코드(job_code), 기좁급여(salary), 인상된 급여 조회
...
- 모든 그룹 함수는 NULL 값을 자동으로 제외하기 때문에
  AVG 함수를 사용할때는 COALESE 또는 IFNULL 함수와 함께 사용하는걸 권장
*/

-- 전체 사원의 평균 급여, 평균 보너스를 조회
SELECT 
...
-- 퇴사한 직원의 수 조회 (ent _date 또는 ent_yn)
