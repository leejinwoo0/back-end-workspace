...
public class ReservationController {
    ...
    public String updateReservation(
            @RequestParam String resCode,          // 예약 코드
            @RequestParam String petsitter,        // 필수 항목
            @RequestParam int petNum,              // 필수 항목
            @RequestParam int careTime,            // 필수 항목
            @RequestParam(required = false) Boolean bathService,  // 선택 항목
    ...
    // 예약 삭제
    @DeleteMapping("/delete/{resCode}")
    public String deleteReservation(@PathVariable String resCode) {
        reservationService.deleteReservation(resCode);
        return "예약이 삭제되었습니다.";
    }
}
